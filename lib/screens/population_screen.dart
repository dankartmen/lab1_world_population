...
class PopulationScreen extends StatelessWidget {
  ...
  // Для интерактивного выбора стран можно добавить:
  Widget _buildCountrySelector(List<String> availableCountries, List<String> selectedCountries, Function(List<String>) onSelectionChanged) {
    return Wrap(
      spacing: 8,
      children: availableCountries.map((country) {
        final isSelected = selectedCountries.contains(country);
        return FilterChip(
          label: Text(_shortenCountryName(country)),
          selected: isSelected,
          onSelected: (selected) {
 ...
 Widget _buildPopulationHistogram(PopulationLoaded state) {
  ...
  Widget _buildContinentChart(PopulationLoaded state) {
    // Создаем график среднего населения по континентам
    final continentData = state.continentAverages;
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Text(
              'Среднее население по континентам',
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
            ),
            SizedBox(height: 16),
            Container(
              ...
              child: BarChart(
                BarChartData(
                  barGroups: continentData.entries.map((e) => BarChartGroupData(
                    x: continentData.keys.toList().indexOf(e.key),
                    barRods: [
                      BarChartRodData(
                        toY: e.value / 1e6, // Конвертируем в миллионы
                        color: _getColor(e.key),
                        width: 20,
                      )
                    ],
                  ...
                  titlesData: FlTitlesData(
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                        showTitles: true,
                        getTitlesWidget: (value, meta) {
                          final continent = continentData.keys.elementAt(value.toInt());
                          return SideTitleWidget(
                            meta: meta,
                            child: Text(_abbreviateContinent(continent)),
                          );
                        },
                      ),
                    ),
        ...
        child: Column(
          children: [
            ...
            Container(
              width: 800,
              height: 500,
              child: _buildChinaIndiaPairPlotChartContent(pairPlotData),
            ),
          ],
        ),
      ),
  ...
  Widget _buildChinaIndiaPairPlotChartContent(List<CountryPopulationData> data) {
    final years = ['1970', '1980', '1990', '2000', '2010', '2015', '2020', '2022'];
    
    return Container(
      width: 800,
      height: 500,
      child: _buildChinaIndiaChart(data, years), 
    );
  }
  ...
  Widget _buildChinaIndiaChart(List<CountryPopulationData> data, List<String> years) {
...
double _calculateComparisonInterval(double maxY) {
  ...
  Color _getColor(String continent) {
    ...
    if (continentLower.contains('north america')) return Colors.amber;
